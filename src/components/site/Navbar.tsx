...
export function Navbar() {
  ...
  const drawerPanel = (
    <div
      className={`fixed inset-0 z-[100] lg:hidden ${drawer ? "" : "pointer-events-none"}`}
      aria-hidden={!drawer}
    >
      <div
      ...
      />
      <aside
        className={`absolute right-0 top-0 flex h-[100dvh] w-[min(100%,380px)] flex-col border-l border-border bg-card shadow-lift transition-transform duration-300 ease-out ${drawer ? "translate-x-0" : "translate-x-full"}`}
        role="dialog"
        aria-modal="true"
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-3 py-4">
          {user && (
            <Link
              ...
              onClick={() => setDrawer(false)}
              className="mb-4 flex items-center gap-3 rounded-2xl bg-muted p-3"
            >
              {profile?.avatar_url ? (
                <img src={profile.avatar_url} className="h-11 w-11 shrink-0 rounded-full object-cover" alt="" />
              ) : (
          ...
          <DrawerGroup label="Browse">
            {NAV.map((i) => (
              <DrawerLink key={i.to} to={i.to} label={i.label} Icon={i.icon} onClick={() => setDrawer(false)} />
            ))}
          </DrawerGroup>
          ...
          {user && (
            <DrawerGroup label="Account">
              ...
              {ACCOUNT.map((a) => (
                <DrawerLink key={a.to} to={a.to} label={a.label} Icon={a.icon} onClick={() => setDrawer(false)} />
              ))}
            </DrawerGroup>
          )}
          ...
          <DrawerGroup label="Explore">
            {MORE.map((m) => (
              <DrawerLink key={m.to} to={m.to} label={m.label} Icon={m.icon} onClick={() => setDrawer(false)} />
            ))}
          </DrawerGroup>
        </div>
        ...
        <div className="shrink-0 border-t border-border p-4">
          {user ? (
            <button
              ...
              className="btn-ghost w-full px-4 py-3 text-sm text-destructive"
            >
              <LogOut className="h-4 w-4" /> Log out
            </button>
          ) : (
            <div className="grid grid-cols-2 gap-2">
              <Link to="/login" onClick={() => setDrawer(false)} className="btn-ghost px-4 py-3 text-sm">
                Login
              </Link>
              <Link to="/signup" onClick={() => setDrawer(false)} className="btn-primary px-4 py-3 text-sm">
                Register
              </Link>
            </div>
          )}
  ...
  return (
    <>
        ...
        <div className="mx-auto flex h-16 max-w-[1400px] items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          ...
          <nav className="hidden min-w-0 items-center gap-1 lg:flex">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" sideOffset={8} className="z-[80] w-60 rounded-2xl border border-border bg-popover p-1.5 shadow-lift">
                {MORE.map((m) => (
          ...
          <div className="hidden items-center gap-3 lg:flex">
            ...
            {user ? (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" sideOffset={8} className="z-[80] w-60 rounded-2xl border border-border bg-popover p-1.5 shadow-lift">
          ...
          <div className="flex items-center gap-1 lg:hidden">
            ...
            <button
              ...
              className="grid h-10 w-10 place-items-center rounded-xl transition hover:bg-muted"
            >
      ...
      {mounted ? createPortal(drawerPanel, document.body) : null}
    </>
  );
}
