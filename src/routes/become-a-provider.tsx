...
export const Route = createFileRoute("/become-a-provider")({
  component: BecomeAProviderLayout,
});

function BecomeAProviderLayout() {
  const pathname = useRouterState({ select: (s) => s.location.pathname });
  const isApply = pathname === "/become-a-provider/apply" || pathname.endsWith("/apply");
  if (isApply) return <Outlet />;
  return <BecomeProviderLanding />;
}

function BecomeProviderLanding() {
  return (
    <PublicLayout>
      <PageHero
        eyebrow="Join Asá"
        title="Turn your skill into a business."
        subtitle="Get discovered, paid securely, and grow with tools built for Nigerian professionals."
      />
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        ...
        <div className="mt-16 grid gap-10 lg:grid-cols-2">
          <div>
            ...
            <ol className="mt-6 space-y-4">
              {[
                ...
                "Complete your profile & upload portfolio",
                "Verify ID and skills",
            ...
            <Link
              to="/become-a-provider/apply"
              className="mt-8 inline-flex items-center gap-2 rounded-full bg-primary px-7 py-3.5 text-sm font-semibold text-primary-foreground hover:opacity-90"
            >
              Apply as Provider
            </Link>
          </div>
          <div className="overflow-hidden rounded-3xl">
            <img
              src="https://images.unsplash.com/photo-1581094794329-c8112a89af12?w=1200"
              alt=""
